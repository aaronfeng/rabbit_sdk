...
C1-C3, A (algo), O (outsider)

say, C1 sends a message M to C2. Who can read the message?
...
- not O
- A
  - needs to see it to do validation, value-add, etc
  - but that means an O who can penetrate the algo system can
    see M
  -> try to keep O outside
  -> do not *store* M unencrypted for long periods of time, to reduce
    the amount of info O can glean

C2 needs to be sure M came from C1.
A is going to provide that guarantee, i.e. C2 doesn't need to verify
C1 directly but just needs to trust A.
A also needs to be sure M came from C1, e.g. for billing and legal
reasons.

use ssl client certs to authenticate clients. make it responsibility
of customers to ensure their private keys are kept private.

how do we stop C1 from successfully authenticating as C1, but then
sending a message claiming to be from C2?
-> get C1 to sign M

same in other direction:
- A has a server cert, C* know public key
- A re-signs messages, so the C* know they came from Algo (NB: no need
  to pass on original sig)

how can C1 be sure that M
- does not get sent to C2
  - trust algo code and system integrity (to prevent tampering w code)
  - algo client at C2 could reject M if it was not destined for
    it. This is a layer that prevents *accidental* disclosure.
- does get sent to C1
  - trust notifications

C*, A are not homogenous entities - they comprise various systems,
